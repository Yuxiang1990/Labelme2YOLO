...
class Labelme2YOLO(object):
    ...
    def convert(self, val_size):
        ...
        for target_dir, json_names in zip(('train/', 'val/'),
            ...
            for json_name in json_names:
                ...
                if json_data['shapes'][0]['shape_type'] == 'polygon':
                    self._save_yolo_label_seg(json_name, self._label_dir_path,
    ...
    def convert_one(self, json_name):
        ...
        if json_data['shape'][0]['shape_type'] == 'polygon':
            self._save_yolo_label_seg(json_name, self._json_dir,
                                  '', yolo_obj_list)
        else:
    ...
    def _get_yolo_object_list(self, json_data, img_path):
        ...
        img_h, img_w, _ = np.array(PIL.Image.open(img_path)).shape
        for shape in json_data['shapes']:
            ...
            if shape['shape_type'] == 'circle':
                ...
                yolo_obj = self._get_seg_yolo_object(shape, img_h, img_w)
            else:
    ...
    def _get_seg_yolo_object(self, shape, img_h, img_w):
        label_id = self._label_id_map[shape['label']]
        result = [label_id]
        # TODO 人为刻意设置，为了保证标签统一
        # result = [1]
        for w, h in shape['points']:
            result.extend([round(float(w / img_w), 6), round(float(h / img_h), 6)])
        return result
    ...
    def _save_yolo_label_seg(self, json_name, label_dir_path, target_dir, yolo_obj_list):
        txt_path = os.path.join(label_dir_path,
                                target_dir,
                                json_name.replace('.json', '.txt'))
        ...
        with open(txt_path, 'w+') as f:
            for yolo_obj_idx, yolo_obj in enumerate(yolo_obj_list):
                series_result = " ".join([str(i) for i in yolo_obj])
                yolo_obj_line = '%s\n' % series_result \
                    if yolo_obj_idx + 1 != len(yolo_obj_list) else \
                    '%s' % series_result
                f.write(yolo_obj_line)
    ...
    def _save_yolo_image(self, json_data, json_name, image_dir_path, target_dir):
        ...
        if os.path.exists(os.path.join(self._json_dir, img_name)):
            shutil.copy(os.path.join(self._json_dir, img_name), img_path)
        else:
